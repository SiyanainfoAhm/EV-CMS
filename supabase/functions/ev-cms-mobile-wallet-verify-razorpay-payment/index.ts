...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const paymentOrderId = String(body.payment_order_id ?? "");
    ...
    const { data, error } = await supabase.rpc("ev_complete_wallet_topup", {
      p_user_id: userId,
    ...
    return json({
      ...
      status: row.status,
      wallet_credited: Boolean(row.wallet_credited),
      failure_reason: row.failure_reason ?? null,
      gateway_name: row.gateway_name ?? "razorpay",
